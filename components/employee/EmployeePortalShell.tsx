...
import { signOutEmployee } from "@/lib/actions/employee-portal";
import type { EmployeePortalReadModel } from "@/types/employee-portal";
    ...
    items: [
      {
        ...
        href: "/employee/sales",
        icon: ShoppingCart,
        keywords: ["sales", "orders", "checkout"],
      },
      {
        key: "inventory",
        label: "Inventory",
      ...
      },
      {
        ...
        icon: ReceiptText,
        keywords: ["claims", "receipts", "spend"],
      },
      {
        key: "tasks",
        label: "Tasks",
        ...
        keywords: ["work", "assignments", "todo"],
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        icon: UserRound,
        keywords: ["account", "security", "details"],
      },
    ],
  },
];
...
function isRouteActive(pathname: string, href: string) {
  ...
  if (targetPath === "/employee") return currentPath === targetPath;

  return currentPath === targetPath || currentPath.startsWith(`${targetPath}/`);
}

function activeKeyFromPath(pathname: string): PortalNavKey {
  if (pathname.startsWith("/employee/sales")) return "sales";
  if (pathname.startsWith("/employee/inventory")) return "inventory";
  if (pathname.startsWith("/employee/expenses")) return "expenses";
  if (pathname.startsWith("/employee/tasks")) return "tasks";
  ...
  return (
    <main className="min-h-screen bg-[var(--app-bg)] text-[var(--text-primary)]">
      <DesktopSidebar
        ...
        employeeRole={summary.role}
        initials={summary.employeeInitials}
        ...
        <header className="sticky top-0 z-30 border-b border-[var(--primary-border)] bg-[var(--navbar-bg)] backdrop-blur-xl">
          <div className="flex h-[76px] items-center justify-between gap-4 px-4 sm:px-6 xl:px-8">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <button
                ...
                aria-label="Search Helix employee portal"
              >
                <Search className="h-4 w-4 shrink-0 text-[var(--text-tertiary)]" />
                <span className="flex-1 text-[11px] text-[var(--text-tertiary)]">Search employee portal</span>
              ...
              <EmployeeNotificationBell notifications={model.notifications} />
              ...
              >
                <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-[var(--primary-border)] bg-[var(--primary-soft)] text-[11px] font-semibold text-[var(--primary)]">
                  {summary.employeeInitials}
                </div>
                <div className="max-w-[145px]">
                  <p className="truncate text-[11px] font-medium text-[var(--text-primary)]">{summary.employeeName}</p>
                  <p className="mt-0.5 truncate text-[9px] text-[var(--text-tertiary)]">{summary.role}</p>
                </div>
              </Link>

              <form action={signOutEmployee} className="hidden lg:block">
                <button
                  type="submit"
          ...
          <div className="flex items-center justify-between border-t border-[var(--border)] px-4 py-2.5 sm:px-6 lg:hidden">
            <div className="flex min-w-0 items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="truncate text-[10px] text-[var(--text-secondary)]">{summary.employeeName}</p>
                <p className="text-[8px] text-[var(--text-muted)]">{summary.role}</p>
              </div>
            </div>
      ...
      <div className="space-y-3 border-t border-[var(--border)] p-4">
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-medium text-[var(--text-primary)]">Employee workspace</p>
              <p className="mt-1 text-[8px] leading-4 text-[var(--text-muted)]">
                Review assigned work, sales, expenses and inventory alerts.
              </p>
            </div>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-2.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-[10px] font-medium text-[var(--text-secondary)]">{employeeName}</p>
            <div className="mt-0.5 flex items-center gap-1.5">
              <span className="h-1.5 w-1.5 rounded-full bg-[var(--success)]" />
              <p className="truncate text-[8px] text-[var(--text-muted)]">
                {employeeRole} · {companyName}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-h-[420px] overflow-y-auto p-2">
          {results.length > 0 ? (
            ...
            <div className="px-4 py-10 text-center">
              ...
              <p className="mt-3 text-[11px] text-[var(--text-tertiary)]">No employee pages found.</p>
            </div>
          )}
        </div>
      </div>
